...
fn get_claim_as_string(key: &str, claims: &HashMap<String, Value>) -> Result<String, ()> {
...
fn get_claim_as_u64(key: &str, claims: &HashMap<String, Value>) -> Result<u64, ()> {
    let Some(fv) = claims.get(key) else {
        error!(claim = key, "invalid jwt claim");
        return Err(());
    };
...
fn get_claim_as_vec(key: &str, claims: &HashMap<String, Value>) -> Result<Vec<String>, ()> {
    let Some(fv) = claims.get(key) else {
        error!(claim = key, "invalid jwt claim");
        return Err(());
    };
