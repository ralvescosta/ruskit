...
impl SqliteDriver {
    pub fn new(pool: Arc<Pool<SqliteConnectionManager>>) -> SqliteDriver {
        SqliteDriver { pool }
    }
}
...
impl SqliteDriver {
    fn get_conn(&self) -> Result<PooledConnection<SqliteConnectionManager>, MigrationError> {
        match self.pool.get() {
            Err(err) => {
                error!(
                    error = err.to_string(),
    ...
    fn begin(
        ...
        conn.prepare("BEGIN TRANSACTION migrations;")
            .map_err(|e| {
